...
export default function PauseButton(props) {
    return (
        <>
            ...
            {props.editMode ? (
                <div className="m-auto w-40 h-40"></div>
            ) : (
                <div
                    className="tooltip tooltip-info tooltip-bottom m-auto w-40"
                    data-tip={props.paused ? 'Restart PiHole' : 'Pause PiHole'}
                >
                    <button
                        className={`m-auto btn-outline ${
                            props.paused ? 'btn-success' : 'btn-error'
                        } rounded-full border-8 w-40 h-40`}
                        onClick={props.togglePaused}
                    >
                        {props.paused ? <RestartIcon /> : <PauseIcon />}
                    </button>
                </div>
            )}
        </>
